...
export default function Home() {
  ...
  return <main>
    ...
    <section className="hero" id="top"><Image src="/images/videoframe_541466.png" alt="" className="hero-image" fill priority sizes="100vw"/><div className="hero-shade"/><div className="hero-copy"><p className="eyebrow">{t.eyebrow}</p><h1>L’ÚLTIM<br/>CLAU</h1><p className="hero-subtitle">{t.subtitle}</p></div><a href="#mirada" className="scroll-cue"><span>{t.scroll}</span><ArrowDown size={18}/></a></section>
    ...
    <section className="moodboard" aria-label="Moodboard">{groups.map((images,i)=><div className={`mood-chapter chapter-${i+1}`} key={i}><div className="chapter-copy"><span>0{i+1}</span><h3>{t.chapters[i][0]}</h3><p>{t.chapters[i][1]}</p></div><div className={`image-composition images-${images.length}`}>{images.map(src=><figure key={src}><Image src={src} alt={alts[lang][alt++]} fill sizes="(max-width: 800px) 100vw, 55vw"/></figure>)}</div></div>)}</section>
